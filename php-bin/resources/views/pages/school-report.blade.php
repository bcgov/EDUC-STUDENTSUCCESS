...
@section('content')
  ...
  <section class="aqua-bg py-4 border-bottom">
    <div class="container">
      ...
      <div class="d-flex justify-content-between align-items-center flex-wrap gap-3 sd-sub-nav">
        ...
        @php
          ...
          if (count($school_report_slugs) > 0) {
            ...
            if ($current_report_index !== 0) {
             print '<a class="btn btn-light btn-lg px-4 shadow-sm fw-bold text-primary border previous" href="/school/'.$school->mincode.'/report/'.$school_report_slugs[$current_report_index - 1].'"><i class="fa fa-angle-left" aria-hidden="true"></i> '.$report_headings[$school_report_slugs[$current_report_index - 1]].'</a>'; 
            }

          @endphp

         <a class="btn btn-light btn-lg px-4 shadow-sm fw-bold text-primary border information" href="/school/{{ $school->mincode }}">{{ trans('esdr2.reports_heading1') }}</a>

         <!-- @php

            if ($current_report_index + 1 < count($school_report_slugs)) {
              print '<a class="btn btn-light btn-lg px-4 shadow-sm fw-bold text-primary border next" href="/school/'.$school->mincode.'/report/'.$school_report_slugs[$current_report_index + 1].'">'.$report_headings[$school_report_slugs[$current_report_index + 1]].' <i class="fa fa-angle-right" aria-hidden="true"></i></a>';
            }
  ...
  <section class="bg-light py-5 fill-viewport">
    <div class="container">
      ...
      @if ($report_slug == 'contextual-information')
        @if ($school->w_enrol1)
          <h3 class="mb-4 text-primary fw-bold">{{ trans('esdr2.2_Enrolment') }}</h3>
        ...
        @endif
        @if ($school->w_enrol2)
          <h3 class="mb-4 text-primary fw-bold">{{ trans('esdr2.3_StudByGrade') }} {{ $metadata['school_contextual_allgrades_yr'] }}</h3>
      ...
      @if ($report_slug == 'fsa')
        ...
        @if ($school->w_fsa1)
          ...
          <h3 class="mb-4 text-primary fw-bold">{{ trans('esdr2.fsa_heading') }}</h3>

          @if($school->fsa_writers_re_4 >= 1)
            <h4 class="mb-3 text-primary fw-bold">{{ trans('esdr2.grade_heading') }} 4</h4>
          ...
          @if($school->fsa_writers_re_7 >= 1)
            <h4 class="mb-3 text-primary fw-bold">{{ trans('esdr2.grade_heading') }} 7</h4>
            @include('components.chart-legend')
        ...
        @if ($school->w_fsa2)
          ...
          <h3 class="mb-4 text-primary fw-bold">{{ trans('esdr2.fsa_heading2') }} - {{ trans('esdr2.fsa_heading_reading') }}</h3>
          ...
          <h3 class="mb-4 text-primary fw-bold">{{ trans('esdr2.fsa_heading3') }} - {{ trans('esdr2.fsa_heading_writing') }}</h3>
          ...
          <h3 class="mb-4 text-primary fw-bold">{{ trans('esdr2.fsa_heading4') }} - {{ trans('esdr2.fsa_heading_numeracy') }}</h3>
      ...
      @if ($report_slug == 'grade-to-grade-transitions' && $school->w_g2g)
        <h3 class="mb-4 text-primary fw-bold">{{ trans('esdr2.g2g_heading') }}</h3>
      ...
      @if ($report_slug == 'student-satisfaction' && $school->w_sat1)
        <h3 class="mb-4 text-primary fw-bold">{{ trans('esdr2.student_sat_heading') }}</h3>
        @include('components.chart-legend')
      ...
      @if ($report_slug == 'post-secondary-career-prep' && $school->w_sat2)
        <h3 class="mb-4 text-primary fw-bold">{{ trans('esdr2.post_sec_heading') }}</h3>
      ...
      @if ($report_slug == 'prov-exams' && $school->w_exam)
        <h3 class="mb-4 text-primary fw-bold">{{ trans('esdr2.prov_exam_heading') }}</h3>
        @include('components.chart-legend')
        ...
        <iframe scrolling="no" id="frameId-12" class="tableau-embed" src="//public.tableau.com/views/ESDR2/13_Assessments?:showVizHome=no&:display_share=no&:embed=true&:toolbar=no&:device=desktop&Mincode={{ $school->mincode }}"></iframe>
        <div style="background:white;margin-top: -41px;position:absolute;height: 27px;width: 100%;"></div>
      @endif

      @if ($report_slug == 'transition-to-post-secondary' && $school->w_psi)
        <h3 class="mb-4 text-primary fw-bold">{{ trans('esdr2.transition_to_post_sec_heading') }}</h3>
